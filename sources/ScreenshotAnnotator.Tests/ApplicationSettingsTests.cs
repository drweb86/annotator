...
public class ApplicationSettingsTests
{
    ...
    public void Load_missing_file_returns_defaults()
    {
        ...
        Assert.False(fs.FileExists(SettingsFilePath));
    ...
    {
        var fs = new FakeFileSystem();
        fs.EnsureDirectoryExists(SettingsDirectoryPath);
        fs.WriteAllText(SettingsFilePath, """
            {
    ...
    public void Save_writes_indented_json_round_trips()
    {
        ...
        Assert.True(fs.FileExists(SettingsFilePath));
        Assert.Contains(SettingsDirectoryPath, fs.CreatedDirectories);
        var raw = fs.ReadAllText(SettingsFilePath);
    ...
    public void Load_invalid_json_returns_defaults()
    {
        var fs = new FakeFileSystem();
        fs.EnsureDirectoryExists(SettingsDirectoryPath);
        fs.WriteAllText(SettingsFilePath, "not json");
