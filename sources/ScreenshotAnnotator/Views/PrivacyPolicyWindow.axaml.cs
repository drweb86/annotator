...
public partial class PrivacyPolicyWindow : Window
{
    ...
    public PrivacyPolicyWindow()
    {
        ...
        LanguageCombo.ItemsSource = DocumentLanguages.Privacy;
        var selected = DocumentStore.LanguageCode() is { } code
            ? DocumentLanguages.ByCode(DocumentLanguages.Privacy, code)
            : DocumentLanguages.MatchDevice(DocumentLanguages.Privacy);
    ...
    private void LoadDocument(DocumentLanguage language)
    {
        MarkdownDocumentPresenter.Render(
            this,
            DocumentHost,
            PrivacyDocuments.LoadMarkdown(language.AssetFile),
            language.Rtl);
    }
}
